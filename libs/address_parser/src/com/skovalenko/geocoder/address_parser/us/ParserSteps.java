...
public class ParserSteps {
	...
	public static AddressToken[] groupNumericWordFields(
		...
		Vector<AddressToken> at = vectroFromArray(addressTokens);
	...
	private static Vector<AddressToken> vectroFromArray(
			AddressToken addressToken[]) {
		Vector<AddressToken> at = new Vector<AddressToken>(addressToken.length);
		for (int j = 0; j < addressToken.length; j++) {
			at.add(addressToken[j]);
		}
		return at;
	}
